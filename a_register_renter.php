...
if(isset($_POST['submit'])){
   ...
   $select_users = $conn->prepare("SELECT * FROM `renters` WHERE email = ?");
   ...
   if($select_users->rowCount() > 0){
      ...
      if($pass != $c_pass){
         ...
         $insert_user = $conn->prepare("INSERT INTO `renters`(id, name, number, email, password) VALUES(?,?,?,?,?)");
         $insert_user->execute([$id, $name, $number, $email, $c_pass]);
         
         if($insert_user){
            $verify_users = $conn->prepare("SELECT * FROM `renters` WHERE email = ? AND password = ? LIMIT 1");
