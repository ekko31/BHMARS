...
<section class="dashboard">
   ...
   <div class="box-container">
      ...
      <div class="box">
      <?php
         $select_profile = $conn->prepare("SELECT * FROM `renters` WHERE id = ? LIMIT 1");
         $select_profile->execute([$user_id]);
         $fetch_profile = $select_profile->fetch(PDO::FETCH_ASSOC);
      ?>
      <h3>welcome!</h3>
      <p><?= $fetch_profile['name']; ?></p>
      <a href="update_renter.php" class="btn">update profile</a>
      </div>
      ...
      <div class="box">
      <?php
        $payment_request = $conn->prepare("SELECT * FROM `payments` WHERE renter = ?");
        $payment_request->execute([$user_id]);
        $total_payment_request = $payment_request->rowCount();
      ?>
      <h3><?= $total_payment_request; ?></h3>
      <p>payment requests from owner</p>
      <a href="payment.php" class="btn">view payment request</a>
      </div>
      ...
      <div class="box">
      <h3>approved</h3>
      <p>list of approved boarding house</p>
      <a href="approve_renter.php" class="btn">Approved Boarding House</a>
      </div>
