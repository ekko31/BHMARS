...
if(isset($_POST['h_search'])){
   ...
   $select_properties = $conn->prepare("SELECT * FROM `property` WHERE address LIKE '%{$location}%' AND type LIKE '%{$type}%' AND offer LIKE '%{$offer}%' AND rooms LIKE '%{$rooms}%' AND status LIKE '%{$status}%' AND price BETWEEN $min AND $max ORDER BY date DESC");
   $select_properties->execute();
