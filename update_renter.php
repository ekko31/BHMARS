...
$select_user = $conn->prepare("SELECT * FROM `renters` WHERE id = ? LIMIT 1");
...
if(isset($_POST['submit'])){
   ...
   if(!empty($name)){
      $update_name = $conn->prepare("UPDATE `renters` SET name = ? WHERE id = ?");
      $update_name->execute([$name, $user_id]);
      $success_msg[] = 'name updated!';
   }

   if(!empty($email)){
      $verify_email = $conn->prepare("SELECT email FROM `renters` WHERE email = ?");
      $verify_email->execute([$email]);
      if($verify_email->rowCount() > 0){
         $warning_msg[] = 'email already taken!';
      }else{
         $update_email = $conn->prepare("UPDATE `renters` SET email = ? WHERE id = ?");
   ...
   if(!empty($number)){
      $verify_number = $conn->prepare("SELECT number FROM `renters` WHERE number = ?");
      $verify_number->execute([$number]);
      if($verify_number->rowCount() > 0){
         $warning_msg[] = 'number already taken!';
      }else{
         $update_number = $conn->prepare("UPDATE `renters` SET number = ? WHERE id = ?");
   ...
   if($old_pass != $empty_pass){
      if($old_pass != $prev_pass){
         ...
         if($new_pass != $empty_pass){
            $update_pass = $conn->prepare("UPDATE `renters` SET password = ? WHERE id = ?");
